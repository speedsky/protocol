...
bool MTcp::Bind(unsigned short port)
{
	sockaddr_in  saddr;
...
int MTcp::Recv(char *buf, int bufsize)
{
	return recv(sock, buf, bufsize, 0);
}
int MTcp::Send(const char *buf, int size)
{
	int s = 0;
	while (s != size)
	{
...
bool MTcp::Connect(const char *ip,unsigned short port,int timeoutms)
{
	sockaddr_in saddr;
	saddr.sin_family = AF_INET;
	saddr.sin_port = htons(port);
...
bool MTcp::SetBlock(bool isblock)
{
